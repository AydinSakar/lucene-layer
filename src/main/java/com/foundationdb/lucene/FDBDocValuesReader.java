...
import static com.foundationdb.lucene.FDBDocValuesWriter.END;
import static com.foundationdb.lucene.FDBDocValuesWriter.FIELD;
import static com.foundationdb.lucene.FDBDocValuesWriter.LENGTH;
import static com.foundationdb.lucene.FDBDocValuesWriter.MAXLENGTH;
import static com.foundationdb.lucene.FDBDocValuesWriter.MINVALUE;
import static com.foundationdb.lucene.FDBDocValuesWriter.NUMVALUES;
import static com.foundationdb.lucene.FDBDocValuesWriter.ORDPATTERN;
import static com.foundationdb.lucene.FDBDocValuesWriter.PATTERN;
import static com.foundationdb.lucene.FDBDocValuesWriter.TYPE;

class FDBDocValuesReader extends DocValuesProducer
{
    ...
    public FDBDocValuesReader(SegmentReadState state, String ext) throws IOException {
        ...
        while(true) {
            ...
            if(dvType == DocValuesType.NUMERIC) {
                ...
                field.dataStartFilePointer = data.getFilePointer();
                data.seek(
                        data.getFilePointer() + (9 + field.pattern
                                                          .length() + field.maxLength) * field.numValues + (1 + field.ordPattern
                                                                                                                     .length()) * maxDoc
                );
            } else {
            ...
            public long get(int docID) {
                try {
                    ...
                    try {
                        ...
                        CorruptIndexException e = new CorruptIndexException(
                                "failed to parse BigDecimal value (resource=" + in + ")"
                        );
                        e.initCause(pe);
                        throw e;
                    }
    ...
    public SortedDocValues getSorted(FieldInfo fieldInfo) throws IOException {
        ...
        return new SortedDocValues()
        {
            ...
            public int getOrd(int docID) {
                ...
                try {
                    in.seek(
                            field.dataStartFilePointer + field.numValues * (9 + field.pattern
                                                                                     .length() + field.maxLength) + docID * (1 + field
                                    .ordPattern
                                    .length())
                    );
                    FDBUtil.readLine(in, scratch);
